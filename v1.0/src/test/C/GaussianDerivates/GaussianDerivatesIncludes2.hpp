float He(const int n,
  ...
  case 2:
    ...
    break;
  case 3:
    v = native_pown(x,3)-3*x;
    break;
  case 4:
    v = native_pown(x,4)-6*x*x+3;
    break;
  case 5:
    v = native_pown(x,5)-10*native_pown(x,3)+15*x;
    break;
  default:
    ;
  }
...
float DaKs(int da[3], float xmy[3], float r, float ks) {
  ...
  float res;
  res = native_pown(-native_sqrt(2.0f)/r, sum_da)*He(da[0],z[0])*He(da[1],z[1])*He(da[2],z[2])*ks;
  return res;
}
