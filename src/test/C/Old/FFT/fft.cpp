...
void four1(double data[], int nn, int isign)
{
  ...
  while (n > mmax) {
    ...
    printf("istep = %d\n", istep);
    printf("n = %d\n", n);
    for (m = 0; m < (mmax>>1); m++) { // ny i hver while iteration; bliver altid 2 gange højere
      // all is used here but in random order
      // #pragma omp parallel for
    printf("FIRST\n");
      for (i = 2*m+1; i <= n; i += istep) { // Nyt startpunkt i hver iteration, step er ikke 1
	    
	printf("i = %d\n", i);
	tempr = wr*data[i + mmax  ] - wi*data[i + mmax+1];
	tempi = wr*data[i + mmax+1] + wi*data[i + mmax  ];
    ...
    printf("SECOND\n");
     for (i = 0; i < n>>2; i++) { // Nyt startpunkt i hver iteration, step er ikke 1
	// printf("i = %d\n", i);
      }
    printf("\n\n");
