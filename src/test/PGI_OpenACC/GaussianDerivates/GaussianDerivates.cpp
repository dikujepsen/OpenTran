...
float DaKs(unsigned da[3], float xmy[3], float r, float ks) {
  ...
  for (unsigned k = 0; k < 3; k++) {
    z[k] = sqrt(2.0)/r*xmy[k];

    sum_da += da[k];
  }
  
  float res;
  res = pow(-sqrtf(2.0)/r, sum_da)*He(da[0],z[0])*He(da[1],z[1])*He(da[2],z[2])*ks;
  return res;
}
...
#ifndef CPU
#ifdef READBACK
#pragma acc kernels loop copyin(p_a_i_x[0:p_a_i_rows * Lp], q_a_i_x[0:q_a_i_rows*Lq], D1Ks__ijb_x[0:D1Ks__ijb_dimsI[1] * dim], D2Ks__ijbg_x[0:D2Ks__ijbg_dimsI[2] * dim], D3Ks__ijbgd_x[0:D3Ks__ijbgd_dimsI[3]*dim], D1Ks__ijb_dimsI[0:2], D2Ks__ijbg_dimsI[0:3], D3Ks__ijbgd_dimsI[0:4])  copyout(K__ij_x[0:K__ij_rows*Lp]) independent
#else
#pragma acc kernels loop copyin(p_a_i_x[0:p_a_i_rows * Lp], q_a_i_x[0:q_a_i_rows*Lq], D1Ks__ijb_x[0:Lq * dim], D2Ks__ijbg_x[0:Lq * dim], D3Ks__ijbgd_x[0:Lq * dim], D1Ks__ijb_dimsI[0:2], D2Ks__ijbg_dimsI[0:3], D3Ks__ijbgd_dimsI[0:4])  local(K__ij_x[0:K__ij_rows*Lp]) independent
#endif
#endif
