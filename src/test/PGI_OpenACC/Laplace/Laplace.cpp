...
#ifndef CPU
#ifdef READBACK
#pragma acc kernels loop copyin(level[0:dim*storagesize], level_int[0:dim*storagesize], index[0:dim*storagesize], alpha[0:storagesize], lcl_q_inv[0:dim], lcl_q[0:dim], lambda[0:dim])  copyout(result[0:storagesize]) independent
#else
#pragma acc kernels loop copyin(level[0:dim*storagesize], level_int[0:dim*storagesize], index[0:dim*storagesize], alpha[0:storagesize], lcl_q_inv[0:dim], lcl_q[0:dim], lambda[0:dim])  local(result[0:storagesize]) independent
#endif
#endif
  for (unsigned i = 0; i < storagesize; i++) {
    double sub = 0.0;
    for (unsigned j = 0; j < storagesize; j++) {
      double gradient_temp[3];
      double dot_temp[3];
	...
	gradient_temp[d] = GRAD(level_i,index_i,
