...
#ifdef USEOCL
#include "finance/algorithm/BlackScholesPATParabolicPDESolverSystemEuroAmerCombined.hpp"
#else
...
#include "parallel/pde/application/BlackScholesSolverOCLMPI.hpp"
...
namespace sg {
  namespace finance {
    ...
    BlackScholesSolverOCLMPI::BlackScholesSolverOCLMPI(bool useLogTransform, bool usePAT)
      : ParabolicPDESolver() {
    ...
    BlackScholesSolverOCLMPI::~BlackScholesSolverOCLMPI() {
    ...
    void BlackScholesSolverOCLMPI::getGridNormalDistribution(DataVector& alpha, std::vector<double>& norm_mu, std::vector<double>& norm_sigma) {
      if (this->bGridConstructed) {
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::getGridNormalDistribution : The grid wasn't initialized before!");
      }
    }

    void BlackScholesSolverOCLMPI::constructGrid(BoundingBox& BoundingBox, int level) {
    ...
    void BlackScholesSolverOCLMPI::refineInitialGridWithPayoff(DataVector& alpha, double strike, std::string payoffType, double dStrikeDistance) {
      ...
      if (this->useLogTransform == false) {
        if (this->bGridConstructed) {
          ...
          if (payoffType == "std_euro_call" || payoffType == "std_euro_put" || payoffType == "std_amer_put") {
            ...
            throw new application_exception("BlackScholesSolverOCLMPI::refineInitialGridWithPayoff : An unsupported payoffType was specified!");
          }
        } else {
          throw new application_exception("BlackScholesSolverOCLMPI::refineInitialGridWithPayoff : The grid wasn't initialized before!");
        }
      }
    }

    void BlackScholesSolverOCLMPI::refineInitialGridWithPayoffToMaxLevel(DataVector& alpha, double strike, std::string payoffType, double dStrikeDistance, sg::base::GridIndex::level_type maxLevel) {
      size_t nRefinements = 0;
      ...
      if (this->useLogTransform == false) {
        if (this->bGridConstructed) {
          ...
          if (payoffType == "std_euro_call" || payoffType == "std_euro_put" || payoffType == "std_amer_put") {
            ...
            throw new application_exception("BlackScholesSolverOCLMPI::refineInitialGridWithPayoffToMaxLevel : An unsupported payoffType was specified!");
          }
        } else {
          throw new application_exception("BlackScholesSolverOCLMPI::refineInitialGridWithPayoffToMaxLevel : The grid wasn't initialized before!");
        }
      }
    }

    void BlackScholesSolverOCLMPI::setStochasticData(DataVector& mus, DataVector& sigmas, DataMatrix& rhos, double r) {
    ...
    void BlackScholesSolverOCLMPI::solveExplicitEuler(size_t numTimesteps, double timestepsize, size_t maxCGIterations, double epsilonCG, DataVector& alpha, bool verbose, bool generateAnimation, size_t numEvalsAnimation) {
      int myrank;
	...
	if (myrank == 0) {
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::solveExplicitEuler : A grid wasn't constructed before or stochastic parameters weren't set!");
      }
    }

    void BlackScholesSolverOCLMPI::solveImplicitEuler(size_t numTimesteps, double timestepsize, size_t maxCGIterations, double epsilonCG, DataVector& alpha, bool verbose, bool generateAnimation, size_t numEvalsAnimation) {
      if (this->bGridConstructed && this->bStochasticDataAlloc) {
	int myrank;
	MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
	...
	if (myrank == 0) {
	  ...
	  std::cout << "Average Grid size (Inner): " << static_cast<double>(myBSSystem->getSumGridPointsInner()) / static_cast<double>(numTimesteps) << std::endl << std::endl << std::endl;
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::solveImplicitEuler : A grid wasn't constructed before or stochastic parameters weren't set!");
      }
    }

    void BlackScholesSolverOCLMPI::solveCrankNicolson(size_t numTimesteps, double timestepsize, size_t maxCGIterations, double epsilonCG, DataVector& alpha, size_t NumImEul) {
...
MPI_Barrier(MPI_COMM_WORLD);
	...
	if (myrank == 0) {
	  ...
	  if (myrank == 0) {
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::solveCrankNicolson : A grid wasn't constructed before or stochastic parameters weren't set!");
      }
    }


    void BlackScholesSolverOCLMPI::solveAdamsBashforth(size_t numTimesteps, double timestepsize, size_t maxCGIterations, double epsilonCG, DataVector& alpha, bool verbose) {
      ODESolver* myODESolver = new AdamsBashforth(numTimesteps, timestepsize, myScreen);
      BlackScholesSolverOCLMPI::solveX(numTimesteps, timestepsize, maxCGIterations, epsilonCG, alpha, verbose, myODESolver, "AdBas");
      delete myODESolver;
    }

    void BlackScholesSolverOCLMPI::solveSC(std::string Solver, size_t numTimesteps, double timestepsize, size_t maxCGIterations, double epsilonCG, DataVector& alpha, bool verbose) {
      std::string tmp;
      ...
      } else std::cerr << "BlackScholesSolverOCLMPI::solveSC(): Unknown Stepsize Control #" << Solver[3] << "#" << Solver << std::endl;

      BlackScholesSolverOCLMPI::solveX(numTimesteps, timestepsize, maxCGIterations, epsilonCG, alpha, verbose, myODESolver, "CrNic");

      if (myODESolver != NULL)
        delete myODESolver;
    }

    void BlackScholesSolverOCLMPI::solveSCAC(size_t numTimesteps, double timestepsize, double epsilon, size_t maxCGIterations, double epsilonCG, DataVector& alpha, bool verbose) {
      ODESolver* myODESolver = new VarTimestep("AdBasC", "CrNic", numTimesteps, timestepsize, epsilon, myScreen, -1);
      BlackScholesSolverOCLMPI::solveX(numTimesteps, timestepsize, maxCGIterations, epsilonCG, alpha, verbose, myODESolver, "CrNic");
      delete myODESolver;
    }

    void BlackScholesSolverOCLMPI::solveSCH(size_t numTimesteps, double timestepsize, double epsilon, size_t maxCGIterations, double epsilonCG, DataVector& alpha, bool verbose) {
      ODESolver* myODESolver = new StepsizeControlH("CrNic", numTimesteps, timestepsize, epsilon, myScreen, 0.9);
      BlackScholesSolverOCLMPI::solveX(numTimesteps, timestepsize, maxCGIterations, epsilonCG, alpha, verbose, myODESolver, "CrNic");
      delete myODESolver;
    }

    void BlackScholesSolverOCLMPI::solveSCBDF(size_t numTimesteps, double timestepsize, double epsilon, size_t maxCGIterations, double epsilonCG, DataVector& alpha, bool verbose) {
      ODESolver* myODESolver = new StepsizeControlBDF(numTimesteps, timestepsize, epsilon, myScreen);
      BlackScholesSolverOCLMPI::solveX(numTimesteps, timestepsize, maxCGIterations, epsilonCG, alpha, verbose, myODESolver, "SCBDF");
      delete myODESolver;
    }

    void BlackScholesSolverOCLMPI::solveSCEJ(size_t numTimesteps, double timestepsize, double epsilon, double myAlpha, size_t maxCGIterations, double epsilonCG, DataVector& alpha, bool verbose) {
      ODESolver* myODESolver = new StepsizeControlEJ("CrNic", numTimesteps, timestepsize, epsilon, myAlpha,  myScreen, 0.5);
      BlackScholesSolverOCLMPI::solveX(numTimesteps, timestepsize, maxCGIterations, epsilonCG, alpha, verbose, myODESolver, "SCEJ");
      delete myODESolver;
    }

    void BlackScholesSolverOCLMPI::solveX(size_t numTimesteps, double timestepsize, size_t maxCGIterations, double epsilonCG, DataVector& alpha, bool verbose, void* myODESolverV, std::string Solver) {
      ...
      if (this->bGridConstructed && this->bStochasticDataAlloc) {
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::solveX : A grid wasn't constructed before or stochastic parameters weren't set!");
      }
    }


    void BlackScholesSolverOCLMPI::initGridWithPayoff(DataVector& alpha, double strike, std::string payoffType) {
      this->dStrike = strike;
      this->payoffType = payoffType;
    ...
    double BlackScholesSolverOCLMPI::get1DEuroCallPayoffValue(double assetValue, double strike) {
    ...
    double BlackScholesSolverOCLMPI::getAnalyticSolution1D(double stock, bool isCall, double t, double vola, double r, double strike) {
    ...
    void BlackScholesSolverOCLMPI::solve1DAnalytic(std::vector< std::pair<double, double> >& premiums, double minStock, double maxStock, double StockInc, double strike, double t, bool isCall) {
      if (bStochasticDataAlloc) {
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::solve1DAnalytic : Stochastic parameters weren't set!");
      }
    }

    void BlackScholesSolverOCLMPI::print1DAnalytic(std::vector< std::pair<double, double> >& premiums, std::string tfilename) {
    ...
    void BlackScholesSolverOCLMPI::getAnalyticAlpha1D(DataVector& alpha_analytic, double strike, double t, std::string payoffType, bool hierarchized) {
      double coord;

      if (dim != 1) {
        throw new application_exception("BlackScholesSolverOCLMPI::getAnalyticAlpha1D : A grid wasn't constructed before!");
      }

      if (!this->bGridConstructed) {
        throw new application_exception("BlackScholesSolverOCLMPI::getAnalyticAlpha1D : function only available for dim = 1!");
      }
    ...
    void BlackScholesSolverOCLMPI::evaluate1DAnalyticCuboid(sg::base::DataVector& AnalyticOptionPrices, sg::base::DataMatrix& EvaluationPoints, double strike, double vola, double r, double t, bool isCall) {
    ...
    std::vector<size_t> BlackScholesSolverOCLMPI::getAlgorithmicDimensions() {
      return this->myGrid->getAlgorithmicDimensions();
    }

    void BlackScholesSolverOCLMPI::setAlgorithmicDimensions(std::vector<size_t> newAlgoDims) {
      if (this->tBoundaryType == "freeBoundaries") {
        this->myGrid->setAlgorithmicDimensions(newAlgoDims);
      } else {
        throw new application_exception("BlackScholesSolverOCLMPI::setAlgorithmicDimensions : Set algorithmic dimensions is only supported when choosing option type all!");
      }
    }

    void BlackScholesSolverOCLMPI::initScreen() {
    ...
    void BlackScholesSolverOCLMPI::setEnableCoarseningData(std::string adaptSolveMode, std::string refineMode, sg::base::GridIndex::level_type refineMaxLevel, int numCoarsenPoints, double coarsenThreshold, double refineThreshold) {
    ...
    void BlackScholesSolverOCLMPI::printPayoffInterpolationError2D(DataVector& alpha, std::string tFilename, size_t numTestpoints, double strike) {
      if (this->useLogTransform == false) {
        if (this->bGridConstructed) {
          ...
          throw new application_exception("BlackScholesSolverOCLMPI::getPayoffInterpolationError : A grid wasn't constructed before!");
        }
      }
    }

    size_t BlackScholesSolverOCLMPI::getGridPointsAtMoney(std::string payoffType, double strike, double eps) {
      size_t nPoints = 0;
        ...
        if (this->bGridConstructed) {
          for (size_t i = 0; i < this->myGrid->getStorage()->size(); i++) {
            ...
            if (payoffType == "std_euro_call" || payoffType == "std_euro_put" || payoffType == "std_amer_put") {
              ...
              throw new application_exception("BlackScholesSolverOCLMPI::getGridPointsAtMoney : An unknown payoff-type was specified!");
            }
          ...
          throw new application_exception("BlackScholesSolverOCLMPI::getGridPointsAtMoney : A grid wasn't constructed before!");
        }
      }
    ...
    void BlackScholesSolverOCLMPI::initCartesianGridWithPayoff(DataVector& alpha, double strike, std::string payoffType) {
      double tmp;
      ...
      if (this->bGridConstructed) {
        for (size_t i = 0; i < this->myGrid->getStorage()->size(); i++) {
          ...
          if (payoffType == "std_euro_call") {
            ...
            throw new application_exception("BlackScholesSolverOCLMPI::initCartesianGridWithPayoff : An unknown payoff-type was specified!");
          }
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::initCartesianGridWithPayoff : A grid wasn't constructed before!");
      }
    }

    void BlackScholesSolverOCLMPI::initLogTransformedGridWithPayoff(DataVector& alpha, double strike, std::string payoffType) {
      double tmp;
      ...
      if (this->bGridConstructed) {
        for (size_t i = 0; i < this->myGrid->getStorage()->size(); i++) {
          ...
          if (payoffType == "std_euro_call") {
            ...
            throw new application_exception("BlackScholesSolverOCLMPI::initLogTransformedGridWithPayoff : An unknown payoff-type was specified!");
          }
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::initLogTransformedGridWithPayoff : A grid wasn't constructed before!");
      }
    }

    void BlackScholesSolverOCLMPI::initPATTransformedGridWithPayoff(DataVector& alpha, double strike, std::string payoffType) {
      double tmp;
      ...
      if (this->bGridConstructed) {
        for (size_t i = 0; i < this->myGrid->getStorage()->size(); i++) {
          ...
          if (payoffType == "std_euro_call") {
            ...
            throw new application_exception("BlackScholesSolverOCLMPI::initPATTransformedGridWithPayoff : An unknown payoff-type was specified!");
          }
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::initPATTransformedGridWithPayoff : A grid wasn't constructed before!");
      }
    }

    double BlackScholesSolverOCLMPI::evalOption(std::vector<double>& eval_point, sg::base::DataVector& alpha) {
    ...
    void BlackScholesSolverOCLMPI::transformPoint(sg::base::DataVector& point) {
    ...
    void BlackScholesSolverOCLMPI::printSparseGridPAT(sg::base::DataVector& alpha, std::string tfilename, bool bSurplus) const {
    ...
    void BlackScholesSolverOCLMPI::resetSolveTime() {
      this->current_time = 0.0;
    }

    size_t BlackScholesSolverOCLMPI::getNeededIterationsToSolve() {
      return this->nNeededIterations;
    }

    double BlackScholesSolverOCLMPI::getNeededTimeToSolve() {
      return this->dNeededTime;
    }

    size_t BlackScholesSolverOCLMPI::getStartInnerGridSize() {
      return this->staInnerGridSize;
    }

    size_t BlackScholesSolverOCLMPI::getFinalInnerGridSize() {
      return this->finInnerGridSize;
    }

    size_t BlackScholesSolverOCLMPI::getAverageInnerGridSize() {
      return this->avgInnerGridSize;
    }

    void BlackScholesSolverOCLMPI::storeInnerMatrix(DataVector& alpha, std::string tFilename, double timestepsize) {
      if (this->bGridConstructed) {
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::storeInnerMatrix : A grid wasn't constructed before!");
      }
    }

    void BlackScholesSolverOCLMPI::storeInnerMatrixDiagonal(DataVector& alpha, std::string tFilename, double timestepsize) {
      if (this->bGridConstructed) {
        OperationParabolicPDESolverSystemDirichlet* myBSSystem = new BlackScholesParabolicPDESolverSystemEuroAmer(*this->myGrid, alpha, *this->mus, *this->sigmas, *this->rhos, this->r, timestepsize, "ImEul", this->dStrike, this->payoffType, this->useLogTransform, this->useCoarsen, this->coarsenThreshold, this->adaptSolveMode, this->numCoarsenPoints, this->refineThreshold, this->refineMode, this->refineMaxLevel);
        SGppStopwatch* myStopwatch = new SGppStopwatch();
    ...
    void BlackScholesSolverOCLMPI::storeInnerMatrixDiagonalRowSum(DataVector& alpha, std::string tFilename, double timestepsize) {
      if (this->bGridConstructed) {
        OperationParabolicPDESolverSystemDirichlet* myBSSystem = new BlackScholesParabolicPDESolverSystemEuroAmer(*this->myGrid, alpha, *this->mus, *this->sigmas, *this->rhos, this->r, timestepsize, "ImEul", this->dStrike, this->payoffType, this->useLogTransform, this->useCoarsen, this->coarsenThreshold, this->adaptSolveMode, this->numCoarsenPoints, this->refineThreshold, this->refineMode, this->refineMaxLevel);
        SGppStopwatch* myStopwatch = new SGppStopwatch();
    ...
    void BlackScholesSolverOCLMPI::storeInnerRHS(DataVector& alpha, std::string tFilename, double timestepsize) {
      if (this->bGridConstructed) {
        OperationParabolicPDESolverSystemDirichlet* myBSSystem = new BlackScholesParabolicPDESolverSystemEuroAmer(*this->myGrid, alpha, *this->mus, *this->sigmas, *this->rhos, this->r, timestepsize, "ImEul", this->dStrike, this->payoffType, this->useLogTransform, this->useCoarsen, this->coarsenThreshold, this->adaptSolveMode, this->numCoarsenPoints, this->refineThreshold, this->refineMode, this->refineMaxLevel);
        SGppStopwatch* myStopwatch = new SGppStopwatch();
    ...
    void BlackScholesSolverOCLMPI::storeInnerSolution(DataVector& alpha, size_t numTimesteps, double timestepsize, size_t maxCGIterations, double epsilonCG, std::string tFilename) {
      if (this->bGridConstructed) {
        ...
        throw new application_exception("BlackScholesSolverOCLMPI::solveImplicitEuler : A grid wasn't constructed before!");
      }
