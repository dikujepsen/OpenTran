...
namespace sg {
  namespace parallel {
	...
	size_t pad2 = 8*LSIZE - storageSizePadded % 8*LSIZE;
	...
	stream_program_src <<	"ptrResult[ii_offset+j] += resTemp[0];		       "  << std::endl;
	stream_program_src << "}" << std::endl;
	stream_program_src << "}" << std::endl;
	...
	stream_program_src <<	"{							       "  << std::endl;
	stream_program_src <<	"unsigned j = get_global_id(1);				       "  << std::endl;
	stream_program_src <<	"unsigned i = get_local_id(0) + get_group_id(0)*8*LSIZE;				       "  << std::endl;
	...
	// stream_program_src <<	"res += ptrSymmetricParResult[j*num_groups + jj_offset+i+4*LSIZE*2];         "  << std::endl;
