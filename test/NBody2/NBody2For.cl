...
__kernel void NBody2For(
	...
	unsigned N, __constant float * ConstantMasPos, __global float * Forces, 
	unsigned hst_ptrPos_dim1)
{
  ...
  for (unsigned j = 0; j < N; j++)
    {
      float b_m = Mas[get_global_id(0)] * ConstantMasPos[(3 * j) + 0];
      float a_x = Pos[(0 * hst_ptrPos_dim1) + get_global_id(0)];
      float a_y = Pos[(1 * hst_ptrPos_dim1) + get_global_id(0)];
      float r_x = ConstantMasPos[(3 * j) + 1] - a_x;
      float r_y = ConstantMasPos[(3 * j) + 2] - a_y;
